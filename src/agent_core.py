...
def run_pharma_agent(
    ...
    if not patient_query.strip():
        return "\n".join(logs), "⚠️ Please enter a clinical query."

    logs.append("🛡️  [GUARDRAIL CHECK — local pre-filter]")
    local_block = local_pii_check(patient_query)
    if local_block:
        logs.append(f"🚫 {local_block}")
        logs.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        return (
            "\n".join(logs),
    ...
    logs.append("✅ Local pre-check passed — forwarding to TrueFoundry Guardrails")
    logs.append("")

    logs.append("🔧 [MCP GATEWAY — OpenFDA Tool]")
    drug_name = extract_drug_name(patient_query)
    logs.append(f"   Extracted drug hint: {drug_name}")
    ...
    if simulate_model_failure:
        logs.append("   Demo mode: show primary failure → gateway fallback recovery")
    else:
    ...
    if gateway_result.blocked_by_guardrail:
        logs.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        logs.append("⛔ BLOCKED BY TRUEFOUNDRY GUARDRAILS")
        logs.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    logs.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logs.append("⚠️  AGENT DEGRADED GRACEFULLY")
    logs.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    return "\n".join(logs), gateway_result.content
